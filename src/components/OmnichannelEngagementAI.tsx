...
import { askOmnichannelEngine } from "../services/omnichannelService";
import { 
  AreaChart, 
  Area, 
  XAxis, 
...
const CONTINUITY_FLOW = [
  { stage: 'Web Initiation', retention: 100 },
  { stage: 'WhatsApp Switch', retention: 85 },
  { stage: 'Email Nurture', retention: 70 },
  { stage: 'CRM Sync', retention: 95 },
];

const COLORS = ['#22d3ee', '#10b981', '#f59e0b', '#8b5cf6', '#ec4899', '#64748b'];

export function OmnichannelEngagementAI({ onNavigate }: { onNavigate: (page: any) => void }) {
  const [messages, setMessages] = useState<{ role: 'user' | 'assistant', content: string, channel?: string, sync?: boolean }[]>([]);
  const [input, setInput] = useState("");
  const [isLoading, setIsLoading] = useState(false);
  const [activeTab, setActiveTab] = useState<'performance' | 'continuity' | 'workflow' | 'conversion' | 'ux' | 'optimization' | 'admin'>('performance');
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const responseText = await askOmnichannelEngine(userMsg, geminiHistory);
      
      // Simulate omnichannel signals
      const channelDetected = responseText.toLowerCase().includes("whatsapp") 
        ? "WHATSAPP" 
      ...
      console.error(err);
      setMessages(prev => [...prev, { role: 'assistant', content: "SYSTEM FAILURE: Omnichannel synchronization node bypassed. Continuity compromised." }]);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[var(--color-bg)] text-[var(--color-text)] flex flex-col font-sans transition-colors duration-1000">
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex flex-col">
            <h1 className="text-xl font-display font-black italic tracking-widest text-white group cursor-default">
              <Trademark text="IPDM Omnichannel AI" />
            </h1>
            <span className="text-[12px] font-mono text-primary font-bold uppercase tracking-wider">Communication Orchestration System</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-12">
            <div className="hidden lg:flex gap-8 items-center border-l border-white/5 pl-8">
               <div className="text-right">
                  <p className="text-[10px] font-mono text-zinc-600 uppercase tracking-wider">Session Continuity</p>
                  <p className="text-[10px] font-bold text-primary tracking-widest">ACTIVE_99.4%</p>
               </div>
               <div className="text-right">
                  <p className="text-[10px] font-mono text-zinc-600 uppercase tracking-wider">Sync Integrity</p>
                  <p className="text-[10px] font-bold text-emerald-500">VERIFIED_STABLE</p>
               </div>
            </div>
            <div className="px-6 py-2 bg-primary/10 border border-primary/20 rounded-xl">
               <div className="flex items-center gap-3">
                  ...
                  <span className="text-[10px] font-mono font-black text-primary uppercase tracking-widest">Omni-Link Live</span>
               </div>
            </div>
        </div>
      </header>
      ...
      <main className="flex-1 max-w-[1920px] mx-auto w-full p-6 lg:p-10 grid grid-cols-1 lg:grid-cols-12 gap-8 overflow-hidden">
        ...
        <aside className="lg:col-span-3 space-y-6 flex flex-col overflow-y-auto pr-2 custom-scrollbar-thin">
           <div className="p-8 rounded-[2.5rem] bg-white/[0.02] border border-white/5 glass">
              <h2 className="text-[10px] font-mono font-black uppercase text-zinc-500 tracking-widest mb-6 flex items-center gap-2">
                <Network size={14} className="text-primary" /> Connected Channels
              </h2>
              <div className="space-y-4">
                 {[
                    { label: 'Web Terminal', status: 'ACTIVE', icon: <Globe size={12} /> },
                    { label: 'WhatsApp Node', status: 'SYNCED', icon: <MessageCircle size={12} /> },
                    { label: 'Email Relay', status: 'LIVE', icon: <Mail size={12} /> },
                    { label: 'Social Bridge', status: 'MONITOR', icon: <Share2 size={12} /> },
                    { label: 'CRM Interface', status: 'READY', icon: <Database size={12} /> },
                 ].map((channel, i) => (
                    <div key={i} className="flex items-center justify-between p-4 bg-white/[0.02] border border-white/5 rounded-2xl group hover:border-primary/40 transition-all">
                       <div className="flex items-center gap-3">
                          <div className="text-zinc-500 group-hover:text-primary transition-colors">{channel.icon}</div>
                          <span className="text-[10px] font-bold text-white uppercase tracking-tight">{channel.label}</span>
                       </div>
                       <span className="text-[8px] font-mono text-primary uppercase tracking-widest">{channel.status}</span>
                    </div>
                 ))}
              </div>
           </div>
           ...
           <div className="p-8 rounded-[2.5rem] bg-white/[0.02] border border-white/5 glass flex-1 relative overflow-hidden group">
              ...
              <h2 className="text-[10px] font-mono font-black uppercase text-zinc-500 tracking-widest mb-6 flex items-center gap-2">
                <Repeat size={14} className="text-primary" /> Continuity Pulse
              </h2>
              <div className="space-y-6">
                 {[
                    { l: 'Context Retention', v: '100%' },
                    { l: 'Response Parity', v: 'EXACT' },
                    { l: 'Bypass Rate', v: '0.0%' },
                 ].map((row, i) => (
              ...
              <div className="mt-8">
                 <div className="h-32 w-full bg-white/[0.02] rounded-3xl border border-white/5 flex items-center justify-center">
                    <Share size={16} className="text-primary/10 w-16 h-16 animate-pulse" />
                 </div>
              </div>
           </div>
        </aside>

        {/* Center: Omnichannel Console */}
           ...
           <div className="flex-1 flex flex-col rounded-[3rem] bg-white/[0.01] border border-white/5 relative overflow-hidden glass shadow-2xl">
              <div className="p-8 border-b border-white/5 flex items-center justify-between bg-black/40">
                 <div className="flex items-center gap-3">
                    ...
                    <div>
                       <span className="text-[11px] font-mono font-black uppercase tracking-[0.3em] text-primary">Unified Bridge</span>
                       <p className="text-[9px] font-mono text-zinc-600 uppercase tracking-widest">Cross-Platform Sync Mode Active</p>
                    </div>
                 </div>
                 <div className="hidden sm:block text-right">
                    <p className="text-[9px] font-mono text-zinc-500 uppercase">Orchestration IQ</p>
                    <p className="text-[10px] font-bold text-primary italic">99.2%_SYNCED</p>
                 </div>
              </div>
              ...
              <div ref={scrollRef} className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar-thin">
                 {messages.length === 0 && (
                    <div className="h-full flex flex-col items-center justify-center text-center space-y-12 py-10">
                       ...
                       <div className="space-y-4">
                          <h2 className="text-3xl font-display font-medium italic tracking-tight text-white">Omnichannel Sync Live</h2>
                          <p className="text-zinc-500 font-mono text-[10px] uppercase tracking-[0.2em] max-w-sm mx-auto leading-loose italic">
                             AI-driven cross-platform engagement infrastructure. 
                             Synchronizing conversations, workflows, and memory across all business touchpoints seamlessly.
                          </p>
                       </div>
                       <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full max-w-lg">
                        {[
                          "Continue this conversation on WhatsApp.",
                          "Send me an email follow-up with the details.",
                          "How do I synchronize my social DMs?",
                          "Bridge this interaction to the CRM."
                        ].map(q => (
                          <button 
                            key={q}
                            onClick={() => setInput(q)}
                         ...
                         {msg.channel && (
                           <div className="flex items-center gap-3 mb-2">
                             <div className="flex items-center gap-1.5 px-2 py-0.5 bg-primary/10 border border-primary/20 rounded">
                               ...
                               <span className="text-[8px] font-mono font-black text-primary uppercase tracking-widest">Routing: {msg.channel}</span>
                             </div>
                             {msg.sync && <span className="text-[8px] font-mono text-zinc-600 uppercase italic">Memory Synchronized</span>}
                           </div>
                         )}
                 ...
                 {isLoading && (
                    <div className="flex gap-6">
                       ...
                       <div className="p-6 rounded-[2rem] bg-white/[0.02] border border-white/5 text-zinc-600 flex items-center gap-4 text-[10px] font-mono uppercase tracking-[0.2em] font-black italic">
                          Synchronizing Cross-Platform Nodes...
                       </div>
                    </div>
                 )}
              </div>
              ...
              <div className="p-8 border-t border-white/5 bg-black/60 backdrop-blur-3xl">
                 <div className="relative max-w-4xl mx-auto group">
                    <input 
                      ...
                      placeholder="ORCHESTRATE OMNICHANNEL ENGAGEMENT..."
        ...
        {/* Right Col: Omnichannel Intelligence Console */}
           ...
           <div className="flex-1 rounded-[3rem] bg-[var(--color-text)]/[0.02] border border-[var(--color-text)]/5 glass flex flex-col overflow-hidden shadow-2xl">
              <div className="p-8 border-b border-white/5 bg-black/40">
                 <div className="flex items-center justify-between mb-8">
                    <div>
                       <h3 className="text-sm font-display font-medium uppercase tracking-[0.2em] italic text-primary">Omnichannel Console</h3>
                       <p className="text-[10px] font-mono text-zinc-600 uppercase tracking-widest mt-1">Cross-Platform Sync & Orchestration Hub</p>
                    </div>
                 ...
                 <div className="flex flex-wrap gap-2">
                    {[
                      { id: 'performance', label: 'Channels', icon: <Globe size={12} /> },
                      { id: 'continuity', label: 'Continuity', icon: <History size={12} /> },
                      { id: 'workflow', label: 'Workflow', icon: <Layers size={12} /> },
                      { id: 'conversion', label: 'Conversion', icon: <Zap size={12} /> },
                      { id: 'ux', label: 'UX', icon: <User size={12} /> },
                   ...
                   >
                     {activeTab === 'performance' && (
                       <div className="space-y-8">
                          <h4 className="text-[10px] font-mono font-black uppercase text-zinc-500 tracking-widest">Engagement by Channel</h4>
                          ...
                          <div className="grid grid-cols-2 gap-4">
                             <div className="p-5 bg-white/[0.02] border border-white/5 rounded-2xl">
                                <p className="text-[9px] font-mono text-zinc-500 uppercase mb-1">Peak Channel</p>
                                <p className="text-xl font-bold text-white italic">Website</p>
                             </div>
                             <div className="p-5 bg-white/[0.02] border border-white/5 rounded-2xl">
                                <p className="text-[9px] font-mono text-zinc-500 uppercase mb-1">Sync Parity</p>
                                <p className="text-xl font-bold text-primary italic">99.4%</p>
                             </div>
                          </div>
                       </div>
                     )}
                     ...
                     {activeTab === 'continuity' && (
                       <div className="space-y-8">
                          <h4 className="text-[10px] font-mono font-black uppercase text-zinc-500 tracking-widest">Cross-Channel Continuity Flow</h4>
                          <div className="h-64 w-full">
                             <ResponsiveContainer width="100%" height="100%">
                                <AreaChart data={CONTINUITY_FLOW}>
                                   ...
                                   <YAxis hide />
                                   <Tooltip contentStyle={{ backgroundColor: '#000', border: 'none', borderRadius: '12px' }} />
                                   <Area type="monotone" dataKey="retention" fill="#22d3ee" fillOpacity={0.1} stroke="#22d3ee" strokeWidth={3} />
                                </AreaChart>
                             </ResponsiveContainer>
                          </div>
                          <div className="space-y-4">
                             {[
                                { l: 'Session Match Rate', v: '100%', s: 'PEAK' },
                                { l: 'Context Synchronization', v: 'SYNCED', s: 'STABLE' },
                                { l: 'Drop-off on Switch', v: 'LOW_2%', s: 'IMPROVED' },
                             ].map((m, i) => (
                     ...
                     {activeTab === 'workflow' && (
                        <div className="space-y-8">
                           <div className="p-8 rounded-[3rem] bg-violet-500/5 border border-violet-500/10 flex flex-col items-center text-center gap-6">
                              <Layers className="text-violet-500" size={48} />
                              <div>
                                 <h4 className="text-lg font-display italic text-white mb-2">Omnichannel Workflow Sync</h4>
                                 <p className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest leading-loose">
                                    Unified orchestration active. Managing cross-platform engagement triggers and multi-channel conversational logic in real time.
                                 </p>
                              </div>
                           </div>
                           <div className="grid grid-cols-1 gap-3">
                              {[
                                 { label: 'Transition Speed', val: 'INSTANT' },
                                 { label: 'Platform Parity', val: 'EXACT' },
                                 { label: 'Sync Integrity', val: 'MATCHED' },
                              ].map((m, i) => (
                     ...
                     {activeTab === 'conversion' && (
                        <div className="space-y-8">
                           <h4 className="text-[10px] font-mono font-black uppercase text-zinc-500 tracking-widest">Cross-Channel Conversion IQ</h4>
                           <div className="space-y-4">
                              {[
                                 { label: 'WhatsApp Conv', rate: 42 },
                                 { label: 'Email Conv', rate: 28 },
                                 { label: 'Web Conv', rate: 35 },
                                 { label: 'Social Conv', rate: 12 },
                              ].map((r, i) => (
                           ...
                           <div className="p-6 bg-white/[0.02] border border-white/5 rounded-3xl text-center">
                              <p className="text-[9px] font-mono text-zinc-500 uppercase mb-2">Omnichannel Conversion IQ</p>
                              <p className="text-3xl font-display text-white italic">94.8%</p>
                           </div>
                        </div>
                     )}
                     ...
                     {activeTab === 'ux' && (
                        <div className="space-y-8">
                           <h4 className="text-[10px] font-mono font-black uppercase text-zinc-500 tracking-widest">Platform UX Consistency</h4>
                           <div className="space-y-4">
                              {[
                                 { label: 'Message Consistency', status: 'VERIFIED', color: 'bg-primary' },
                                 { label: 'Tone Synchronization', status: 'ACTIVE', color: 'bg-emerald-500' },
                                 { label: 'Recall Accuracy', status: 'PEAK', color: 'bg-indigo-500' },
                              ].map((e, i) => (
                           ...
                           <div className="p-6 bg-primary/5 border border-primary/20 rounded-2xl flex items-center gap-4 group cursor-pointer hover:bg-primary/10 transition-all">
                              ...
                              <p className="text-[10px] font-mono text-zinc-400 italic">User continuity preserved across <span className="text-white font-bold">ALL CHANNELS</span>.</p>
                           </div>
                        </div>
                     )}
                     ...
                     {activeTab === 'optimization' && (
                        <div className="space-y-8">
                           <h4 className="text-[10px] font-mono font-black uppercase text-zinc-500 tracking-widest">Orchestration IQ Optimization</h4>
                           <div className="h-56 w-full">
                              <ResponsiveContainer width="100%" height="100%">
                                 <RadarChart cx="50%" cy="50%" outerRadius="80%" data={[
                                   { subject: 'Routing', A: 95, fullMark: 100 },
                                   { subject: 'Recall', A: 98, fullMark: 100 },
                                   { subject: 'Sync', A: 100, fullMark: 100 },
                                   { subject: 'Continuity', A: 92, fullMark: 100 },
                           ...
                           <div className="grid grid-cols-2 gap-4">
                              <div className="p-6 bg-white/[0.02] border border-white/5 rounded-3xl text-center">
                                 <p className="text-[9px] font-mono text-zinc-500 uppercase mb-1">Consistency IQ</p>
                                 <p className="text-2xl font-display text-white">99.4/100</p>
                              </div>
                              <div className="p-6 bg-white/[0.02] border border-white/5 rounded-3xl text-center">
                                 <p className="text-[9px] font-mono text-zinc-500 uppercase mb-1">Conv Lift</p>
                                 <p className="text-2xl font-display text-primary">+22.4%</p>
                              </div>
                           </div>
                        </div>
                     )}
                     ...
                     {activeTab === 'admin' && (
                        <div className="space-y-4">
                           {[
                              "Configure Channel Routing Nodes",
                              "Edit Omnichannel Workflow Rules",
                              "Manage Unified Memory Schema",
