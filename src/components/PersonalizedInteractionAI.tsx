...
import { askPersonalizationEngine } from "../services/personalizationService";
import { 
  AreaChart, 
  Area, 
  XAxis, 
...
const ADAPTATION_DATA = [
  ...
  { name: 'Vocabulary', value: 95 },
  { name: 'Pacing', value: 84 },
  { name: 'Format', value: 90 },
];

const PREFERENCE_CLUSTERS = [
...
export function PersonalizedInteractionAI({ onNavigate }: { onNavigate: (page: any) => void }) {
  const [messages, setMessages] = useState<{ role: 'user' | 'assistant', content: string, style?: string, adaptation?: number }[]>([]);
  const [input, setInput] = useState("");
  const [isLoading, setIsLoading] = useState(false);
  const [activeTab, setActiveTab] = useState<'metrics' | 'behavior' | 'workflow' | 'recommendations' | 'continuity' | 'optimization' | 'admin'>('metrics');
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const responseText = await askPersonalizationEngine(userMsg, geminiHistory);
      
      // Simulate personalization signals
      const adaptation = Math.floor(Math.random() * 5) + 94; 
      const styleDetected = userMsg.toLowerCase().includes("technical") || userMsg.toLowerCase().includes("how to")
        ? "TECHNICAL_ADVISOR" 
        : userMsg.toLowerCase().includes("strategy") || userMsg.toLowerCase().includes("growth")
      ...
      console.error(err);
      setMessages(prev => [...prev, { role: 'assistant', content: "SYSTEM FAILURE: Personalization node decoupled. Defaulting to generic engagement mode." }]);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[var(--color-bg)] text-[var(--color-text)] flex flex-col font-sans transition-colors duration-1000">
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex flex-col">
            <h1 className="text-xl font-display font-black italic tracking-widest text-white group cursor-default">
              <Trademark text="IPDM Personalization AI" />
            </h1>
            <span className="text-[12px] font-mono text-primary font-bold uppercase tracking-wider">Adaptive Engagement Engine</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-12">
            <div className="hidden lg:flex gap-8 items-center border-l border-white/5 pl-8">
               <div className="text-right">
                  <p className="text-[10px] font-mono text-zinc-600 uppercase tracking-wider">Personalization Accuracy</p>
                  <p className="text-[10px] font-bold text-primary tracking-widest">VERIFIED_98.4%</p>
               </div>
               <div className="text-right">
                  <p className="text-[10px] font-mono text-zinc-600 uppercase tracking-wider">User Adaptation</p>
                  <p className="text-[10px] font-bold text-emerald-500">DYNAMIC_ACTIVE</p>
               </div>
            </div>
            <div className="px-6 py-2 bg-primary/10 border border-primary/20 rounded-xl">
               <div className="flex items-center gap-3">
                  ...
                  <span className="text-[10px] font-mono font-black text-primary uppercase tracking-widest">Profile Synchronized</span>
               </div>
            </div>
        </div>
      </header>
      ...
      <main className="flex-1 max-w-[1920px] mx-auto w-full p-6 lg:p-10 grid grid-cols-1 lg:grid-cols-12 gap-8 overflow-hidden">
        ...
        <aside className="lg:col-span-3 space-y-6 flex flex-col overflow-y-auto pr-2 custom-scrollbar-thin">
           <div className="p-8 rounded-[2.5rem] bg-white/[0.02] border border-white/5 glass">
              <h2 className="text-[10px] font-mono font-black uppercase text-zinc-500 tracking-widest mb-6 flex items-center gap-2">
                <UserCircle size={14} className="text-primary" /> Active User Profile
              </h2>
              <div className="space-y-4">
                 {[
                    { label: 'Decision Style', value: 'STRATEGIC', icon: <Target size={12} /> },
                    { label: 'Technical Depth', value: 'MODERATE', icon: <Cpu size={12} /> },
                    { label: 'Engagement Tone', value: 'PROFESSIONAL', icon: <MessageSquare size={12} /> },
                    { label: 'Interest Area', value: 'AI_OPS', icon: <Database size={12} /> },
                    { label: 'Growth Intent', value: 'HIGH', icon: <TrendingUp size={12} /> },
                 ].map((stat, i) => (
           ...
           <div className="p-8 rounded-[2.5rem] bg-white/[0.02] border border-white/5 glass flex-1 relative overflow-hidden group">
              ...
              <h2 className="text-[10px] font-mono font-black uppercase text-zinc-500 tracking-widest mb-6 flex items-center gap-2">
                <BrainCircuit size={14} className="text-primary" /> Cognitive Sync
              </h2>
              <div className="space-y-6">
                 {[
                    { l: 'Adaptation Speed', v: 'INSTANT' },
                    { l: 'Learning Rate', v: '99.2%' },
                    { l: 'Context Depth', v: 'DEEP_CORE' },
                 ].map((row, i) => (
              ...
              <div className="mt-8">
                 <div className="h-32 w-full bg-white/[0.02] rounded-3xl border border-white/5 flex items-center justify-center">
                    <History className="text-primary/10 w-16 h-16 animate-pulse" />
                 </div>
              </div>
           </div>
        </aside>

        {/* Center: Personalized Interaction Console */}
           ...
           <div className="flex-1 flex flex-col rounded-[3rem] bg-white/[0.01] border border-white/5 relative overflow-hidden glass shadow-2xl">
              <div className="p-8 border-b border-white/5 flex items-center justify-between bg-black/40">
                 <div className="flex items-center gap-3">
                    ...
                    <div>
                       <span className="text-[11px] font-mono font-black uppercase tracking-[0.3em] text-primary">Adaptive Bridge</span>
                       <p className="text-[9px] font-mono text-zinc-600 uppercase tracking-widest">Personalized Interaction Logic Active</p>
                    </div>
                 </div>
                 <div className="hidden sm:block text-right">
                    <p className="text-[9px] font-mono text-zinc-500 uppercase">Personalization IQ</p>
                    <p className="text-[10px] font-bold text-primary italic">99.8%_MATCH</p>
                 </div>
              </div>
              ...
              <div ref={scrollRef} className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar-thin">
                 {messages.length === 0 && (
                    <div className="h-full flex flex-col items-center justify-center text-center space-y-12 py-10">
                       ...
                       <div className="space-y-4">
                          <h2 className="text-3xl font-display font-medium italic tracking-tight text-white">Engagement Link Individualized</h2>
                          <p className="text-zinc-500 font-mono text-[10px] uppercase tracking-[0.2em] max-w-sm mx-auto leading-loose italic">
                             AI-driven personalized engagement infrastructure. 
                             Dynamically adapting communication and workflows to your unique profile.
                          </p>
                       </div>
                         ...
                         {msg.style && (
                           <div className="flex items-center gap-3 mb-2">
                             <div className="flex items-center gap-1.5 px-2 py-0.5 bg-primary/10 border border-primary/20 rounded">
                               ...
                               <span className="text-[8px] font-mono font-black text-primary uppercase tracking-widest">{msg.style}</span>
                             </div>
                             <span className="text-[8px] font-mono text-zinc-600 uppercase italic">Adaptation: {msg.adaptation}%</span>
                           </div>
                         )}
                 ...
                 {isLoading && (
                    <div className="flex gap-6">
                       ...
                       <div className="p-6 rounded-[2rem] bg-white/[0.02] border border-white/5 text-zinc-600 flex items-center gap-4 text-[10px] font-mono uppercase tracking-[0.2em] font-black italic">
                          Synchronizing Personalization Nodes...
                       </div>
                    </div>
                 )}
              </div>
              ...
              <div className="p-8 border-t border-white/5 bg-black/60 backdrop-blur-3xl">
                 <div className="relative max-w-4xl mx-auto group">
                    <input 
                      ...
                      placeholder="ENGAGE WITH ADAPTIVE AI..."
        ...
        {/* Right Col: Personalization Intelligence Console */}
           ...
           <div className="flex-1 rounded-[3rem] bg-[var(--color-text)]/[0.02] border border-[var(--color-text)]/5 glass flex flex-col overflow-hidden shadow-2xl">
              <div className="p-8 border-b border-white/5 bg-black/40">
                 <div className="flex items-center justify-between mb-8">
                    <div>
                       <h3 className="text-sm font-display font-medium uppercase tracking-[0.2em] italic text-primary">Personalization Console</h3>
                       <p className="text-[10px] font-mono text-zinc-600 uppercase tracking-widest mt-1">Adaptive Engagement Intelligence Hub</p>
                    </div>
                 ...
                 <div className="flex flex-wrap gap-2">
                    {[
                      ...
                      { id: 'recommendations', label: 'Guidance', icon: <Sparkles size={12} /> },
                   ...
                   >
                     {activeTab === 'metrics' && (
                       <div className="space-y-8">
                          <h4 className="text-[10px] font-mono font-black uppercase text-zinc-500 tracking-widest">Adaptation Distribution</h4>
                          <div className="h-64 w-full">
                             <ResponsiveContainer width="100%" height="100%">
                                <ReBarChart data={ADAPTATION_DATA}>
                                   <XAxis dataKey="name" tick={{ fill: '#666', fontSize: 8, fontWeight: 900 }} />
                                   <YAxis hide />
                                   <Tooltip contentStyle={{ backgroundColor: '#000', border: 'none', borderRadius: '12px' }} />
                                   <Bar dataKey="value" fill="#22d3ee" radius={[4, 4, 0, 0]}>
                                      {ADAPTATION_DATA.map((entry, index) => (
                          ...
                          <div className="grid grid-cols-2 gap-4">
                             <div className="p-5 bg-white/[0.02] border border-white/5 rounded-2xl">
                                <p className="text-[9px] font-mono text-zinc-500 uppercase mb-1">Personalization Accuracy</p>
                                <p className="text-xl font-bold text-white italic">98.4%</p>
                             </div>
                             <div className="p-5 bg-white/[0.02] border border-white/5 rounded-2xl">
                                <p className="text-[9px] font-mono text-zinc-500 uppercase mb-1">Adaptation Peak</p>
                                <p className="text-xl font-bold text-primary italic">99.8%</p>
                             </div>
                          </div>
                       </div>
                     )}
                     ...
                     {activeTab === 'behavior' && (
                       <div className="space-y-8">
                          <h4 className="text-[10px] font-mono font-black uppercase text-zinc-500 tracking-widest">User Preference Clusters</h4>
                          <div className="h-64 w-full">
                             <ResponsiveContainer width="100%" height="100%">
                                <PieChart>
                                   <Pie
                                     data={PREFERENCE_CLUSTERS}
                                     cx="50%"
                                     cy="50%"
                                     innerRadius={60}
                                     outerRadius={80}
                                     ...
                                     dataKey="value"
                                   >
                                     {PREFERENCE_CLUSTERS.map((entry, index) => (
                                       <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />
                                     ))}
                                   </Pie>
                                   <Tooltip contentStyle={{ backgroundColor: '#000', border: 'none', borderRadius: '12px' }} />
                          ...
                          <div className="space-y-4">
                             {[
                                { l: 'Strategic Focus', v: '92%', s: 'PEAK' },
                                { l: 'Technical Interest', v: 'LOW', s: 'MONITORING' },
                                { l: 'Conversion Potential', v: 'HIGH', s: 'STABLE' },
                             ].map((m, i) => (
                     ...
                     {activeTab === 'workflow' && (
                        <div className="space-y-8">
                           <div className="p-8 rounded-[3rem] bg-cyan-500/5 border border-cyan-500/10 flex flex-col items-center text-center gap-6">
                              ...
                              <div>
                                 <h4 className="text-lg font-display italic text-white mb-2">Adaptive Workflow Orchestration</h4>
                                 <p className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest leading-loose">
                                    Dynamic journey management active. Conversational sequencing and CTA timing synchronized with behavioral readiness.
                                 </p>
                              </div>
                           </div>
                           <div className="grid grid-cols-1 gap-3">
                              {[
                                 { label: 'Journey Match', val: '99.1%' },
                                 { label: 'CTA Relevance', val: 'EXACT' },
                                 { label: 'Friction Level', val: 'MINIMAL' },
                              ].map((m, i) => (
                     ...
                     {activeTab === 'recommendations' && (
                        <div className="space-y-8">
                           <h4 className="text-[10px] font-mono font-black uppercase text-zinc-500 tracking-widest">Recommendation Relevance</h4>
                           <div className="space-y-4">
                              {[
                                 { label: 'Solution Mapping', rate: 96 },
                                 { label: 'Goal Alignment', rate: 98 },
                                 { label: 'Industry Specificity', rate: 92 },
                                 { label: 'Acceptance Probability', rate: 88 },
                              ].map((r, i) => (
                           ...
                           <div className="p-6 bg-white/[0.02] border border-white/5 rounded-3xl text-center">
                              <p className="text-[9px] font-mono text-zinc-500 uppercase mb-2">Personalized Recommendation IQ</p>
                              <p className="text-3xl font-display text-white italic">99.4%</p>
                           </div>
                        </div>
                     )}
                     ...
                     {activeTab === 'continuity' && (
                        <div className="space-y-8">
                           <h4 className="text-[10px] font-mono font-black uppercase text-zinc-500 tracking-widest">Interaction Continuity Sync</h4>
                           <div className="space-y-4">
                              {[
                                 { label: 'Preference Memory', status: 'ACTIVE', color: 'bg-primary' },
                                 { label: 'History Persistence', status: 'SYNCED', color: 'bg-emerald-500' },
                                 { label: 'Contextual Recall', status: 'LIVE', color: 'bg-indigo-500' },
                              ].map((e, i) => (
                           ...
                           <div className="p-6 bg-primary/5 border border-primary/20 rounded-2xl flex items-center gap-4 group cursor-pointer hover:bg-primary/10 transition-all">
                              ...
                              <p className="text-[10px] font-mono text-zinc-400 italic">User continuity preserved across <span className="text-white font-bold">12 Interaction Layers</span>.</p>
                           </div>
                        </div>
                     )}
                     ...
                     {activeTab === 'optimization' && (
                        <div className="space-y-8">
                           <h4 className="text-[10px] font-mono font-black uppercase text-zinc-500 tracking-widest">Personalization IQ Optimization</h4>
                           <div className="h-56 w-full">
                              <ResponsiveContainer width="100%" height="100%">
                                 <RadarChart cx="50%" cy="50%" outerRadius="80%" data={[
                                   { subject: 'Adaptation', A: 95, fullMark: 100 },
                                   { subject: 'Recall', A: 98, fullMark: 100 },
                                   { subject: 'Tone', A: 100, fullMark: 100 },
                                   { subject: 'Flow', A: 92, fullMark: 100 },
                                   { subject: 'Intent', A: 85, fullMark: 100 },
                                 ]}>
                           ...
                           <div className="grid grid-cols-2 gap-4">
                              <div className="p-6 bg-white/[0.02] border border-white/5 rounded-3xl text-center">
                                 <p className="text-[9px] font-mono text-zinc-500 uppercase mb-1">Customization IQ</p>
                                 <p className="text-2xl font-display text-white">99.8/100</p>
                              </div>
                              <div className="p-6 bg-white/[0.02] border border-white/5 rounded-3xl text-center">
                                 <p className="text-[9px] font-mono text-zinc-500 uppercase mb-1">Engagement lift</p>
                                 <p className="text-2xl font-display text-primary">+24.2%</p>
                              </div>
                           </div>
                        </div>
                     )}
                     ...
                     {activeTab === 'admin' && (
                        <div className="space-y-4">
                           {[
                              "Configure Personalization Thresholds",
                              "Edit Adaptive Communication Library",
                              "Manage User Preference Schema",
                              "Configure Recommendation Logic",
                              "Manage Continuity Sync Protocols",
                              "Audit Personalization Analytics"
                           ].map((btn, i) => (
