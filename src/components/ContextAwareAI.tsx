...
export function ContextAwareAI({ onNavigate }: { onNavigate: (page: any) => void }) {
  const [messages, setMessages] = useState<{ role: 'user' | 'assistant', content: string, confidence?: number, contextType?: string }[]>([]);
  const [input, setInput] = useState("");
  const [isLoading, setIsLoading] = useState(false);
  const [activeTab, setActiveTab] = useState<'metrics' | 'quality' | 'hallucination' | 'personalization' | 'escalation' | 'optimization' | 'admin'>('metrics');
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const contextType = userMsg.length > 30 ? 'Structured Knowledge' : 'Conversational Memory';
      ...
      console.error(err);
      setMessages(prev => [...prev, { role: 'assistant', content: "SYSTEM FAILURE: Contextual node offline. Grounding lost." }]);
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex flex-col">
            <h1 className="text-xl font-display font-black italic tracking-widest text-white group cursor-default">
              <Trademark text="IPDM Context AI" />
            </h1>
            <span className="text-[12px] font-mono text-primary font-bold uppercase tracking-wider">Conversational OS</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-12">
            <div className="hidden lg:flex gap-8 items-center border-l border-white/5 pl-8">
               <div className="text-right">
                  <p className="text-[10px] font-mono text-zinc-600 uppercase tracking-wider">Grounding Parity</p>
                  <p className="text-[10px] font-bold text-primary tracking-widest">VERIFIED_99.8%</p>
               </div>
               <div className="text-right">
                  <p className="text-[10px] font-mono text-zinc-600 uppercase tracking-wider">Hallucination Risk</p>
                  <p className="text-[10px] font-bold text-emerald-500">NEGIGIBLE_0.01%</p>
               </div>
            </div>
            <div className="px-6 py-2 bg-primary/10 border border-primary/20 rounded-xl">
               <div className="flex items-center gap-3">
                  ...
                  <span className="text-[10px] font-mono font-black text-primary uppercase tracking-widest">Context Synchronized</span>
               </div>
            </div>
        </div>
      </header>
        ...
        <aside className="lg:col-span-3 space-y-6 flex flex-col overflow-y-auto pr-2 custom-scrollbar-thin">
           <div className="p-8 rounded-[2.5rem] bg-white/[0.02] border border-white/5 glass">
              ...
              <div className="space-y-4">
                 {[
                    { label: 'Business Logic', status: 'GROUNDED', icon: <Briefcase size={12} /> },
                    { label: 'Session Memory', status: 'ACTIVE', icon: <MemoryStick size={12} /> },
                    { label: 'User Persona', status: 'MAPPED', icon: <Fingerprint size={12} /> },
                    { label: 'Workflow State', status: 'LOCKED', icon: <Layers size={12} /> },
                    { label: 'Escalation Node', status: 'STANDBY', icon: <AlertTriangle size={12} /> },
                 ].map((layer, i) => (
                    <div key={i} className="flex items-center justify-between p-4 bg-white/[0.02] border border-white/5 rounded-2xl group hover:border-primary/40 transition-all">
                       <div className="flex items-center gap-3">
                          <div className="text-zinc-500 group-hover:text-primary transition-colors">{layer.icon}</div>
                          <span className="text-[10px] font-bold text-white uppercase tracking-tight">{layer.label}</span>
                       </div>
                       <span className="text-[8px] font-mono text-primary uppercase tracking-widest">{layer.status}</span>
                    </div>
                 ))}
              </div>
           </div>
           ...
           <div className="flex-1 flex flex-col rounded-[3rem] bg-white/[0.01] border border-white/5 relative overflow-hidden glass shadow-2xl">
              <div className="p-8 border-b border-white/5 flex items-center justify-between bg-black/40">
                 <div className="flex items-center gap-3">
                    ...
                    <div>
                       <span className="text-[11px] font-mono font-black uppercase tracking-[0.3em] text-primary">Contextual AI Bridge</span>
                       <p className="text-[9px] font-mono text-zinc-600 uppercase tracking-widest">Stateful Operating System Active</p>
                    </div>
                 </div>
                 <div className="hidden sm:block text-right">
                    <p className="text-[9px] font-mono text-zinc-500 uppercase">Latency</p>
                    <p className="text-[10px] font-bold text-primary italic">84ms_FAST</p>
                 </div>
              </div>
              ...
              <div ref={scrollRef} className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar-thin">
                 {messages.length === 0 && (
                    <div className="h-full flex flex-col items-center justify-center text-center space-y-12 py-10">
                       ...
                       <div className="space-y-4">
                          <h2 className="text-3xl font-display font-medium italic tracking-tight text-white">Context Link Initialized</h2>
                         ...
                         {msg.contextType && (
                           <div className="flex items-center gap-3 mb-2">
                             <div className="flex items-center gap-1.5 px-2 py-0.5 bg-primary/10 border border-primary/20 rounded">
                               ...
                               <span className="text-[8px] font-mono font-black text-primary uppercase tracking-widest">{msg.contextType}</span>
                             </div>
                             <span className="text-[8px] font-mono text-zinc-600 uppercase italic">Grounding: {msg.confidence}%</span>
                           </div>
                         )}
           ...
           <div className="flex-1 rounded-[3rem] bg-[var(--color-text)]/[0.02] border border-[var(--color-text)]/5 glass flex flex-col overflow-hidden shadow-2xl">
              <div className="p-8 border-b border-white/5 bg-black/40">
                 <div className="flex items-center justify-between mb-8">
                    <div>
                       <h3 className="text-sm font-display font-medium uppercase tracking-[0.2em] italic text-primary">Context Console</h3>
                       <p className="text-[10px] font-mono text-zinc-600 uppercase tracking-widest mt-1">Grounding & Continuity Hub</p>
                    </div>
                 ...
                 <div className="flex flex-wrap gap-2">
                    {[
                      { id: 'metrics', label: 'Metrics', icon: <Activity size={12} /> },
                      { id: 'quality', label: 'Quality', icon: <CheckCircle2 size={12} /> },
                      { id: 'hallucination', label: 'Grounding', icon: <ShieldCheck size={12} /> },
                      { id: 'personalization', label: 'Advisory', icon: <Target size={12} /> },
                      { id: 'escalation', label: 'Escalation', icon: <AlertTriangle size={12} /> },
                      { id: 'optimization', label: 'Optimize', icon: <Zap size={12} /> },
                   ...
                   >
                     {activeTab === 'metrics' && (
                       <div className="space-y-8">
                          <h4 className="text-[10px] font-mono font-black uppercase text-zinc-500 tracking-widest">Grounding Parity Scopes</h4>
                          <div className="h-64 w-full">
                             <ResponsiveContainer width="100%" height="100%">
                                <RadarChart cx="50%" cy="50%" outerRadius="80%" data={CONTEXT_ACCURACY}>
                                   <PolarGrid stroke="rgba(255,255,255,0.05)" />
                                   <PolarAngleAxis dataKey="subject" tick={{ fill: '#666', fontSize: 8, fontWeight: 900 }} />
                                   <Radar name="Context" dataKey="A" stroke="#22d3ee" fill="#22d3ee" fillOpacity={0.2} />
                          ...
                          <div className="grid grid-cols-2 gap-4">
                             <div className="p-5 bg-white/[0.02] border border-white/5 rounded-2xl">
                                <p className="text-[9px] font-mono text-zinc-500 uppercase mb-1">Retention Score</p>
                                <p className="text-xl font-bold text-white italic">99.4/100</p>
                             </div>
                             <div className="p-5 bg-white/[0.02] border border-white/5 rounded-2xl">
                                <p className="text-[9px] font-mono text-zinc-500 uppercase mb-1">Retrieval Latency</p>
                                <p className="text-xl font-bold text-primary italic">0.08ms</p>
                             </div>
                          </div>
                       </div>
                     )}
                     ...
                     {activeTab === 'quality' && (
                       <div className="space-y-8">
                          <h4 className="text-[10px] font-mono font-black uppercase text-zinc-500 tracking-widest text-center">Conversation Continuity Flow</h4>
                          <div className="h-48 w-full">
                             <ResponsiveContainer width="100%" height="100%">
                                <ReLineChart data={CONTINUITY_FLOW}>
                                   <Line type="monotone" dataKey="continuity" stroke="#22d3ee" strokeWidth={3} dot={{ r: 4, fill: '#22d3ee' }} />
                          ...
                          <div className="space-y-3">
                             {[
                                ...
                                <div key={i} className="p-4 bg-white/[0.02] border border-white/5 rounded-2xl flex justify-between items-center">
                                   <span className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest italic">{q.l}</span>
                                   <span className="text-sm font-bold text-white">{q.v}</span>
                                </div>
                             ))}
                          </div>
                       </div>
                     ...
                     {activeTab === 'hallucination' && (
                        <div className="space-y-8">
                           <div className="p-8 rounded-[3rem] bg-emerald-500/5 border border-emerald-500/10 flex flex-col items-center text-center gap-6">
                              ...
                              <div>
                                 <h4 className="text-lg font-display italic text-white mb-2">Zero-Hallucination Lock</h4>
                                 <p className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest leading-loose">
                                    All responses are grounded in approved business operations framework. Grounding verification active across all sessions.
                                 </p>
                              </div>
                           </div>
                           <div className="space-y-4">
                              {[
                                 { label: 'Unsupported Detection', success: '124 Stopped' },
                                 { label: 'Fact Verification', success: '99.9% Parity' },
                                 { label: 'Boundary Enforcement', success: 'LOCKED' },
                              ].map((m, i) => (
                                 <div key={i} className="flex justify-between items-center text-[10px] font-mono uppercase tracking-widest text-zinc-500">
                                    <span>{m.label}</span>
                                    <span className="text-primary font-bold">{m.success}</span>
                                 </div>
                              ))}
                           </div>
                        </div>
                     ...
                     {activeTab === 'personalization' && (
                        <div className="space-y-6">
                           <h4 className="text-[10px] font-mono font-black uppercase text-zinc-500 tracking-widest">Personalization IQ</h4>
                           <div className="grid grid-cols-1 gap-4">
                              {[
                                 ...
                                 <div key={i} className="p-5 bg-white/[0.02] border border-white/5 rounded-3xl flex justify-between items-center group hover:bg-white/5 transition-all">
                                    <span className="text-[10px] font-mono text-zinc-400 uppercase italic">{p.l}</span>
                                    <span className="text-[10px] font-mono font-black text-primary uppercase">{p.v}</span>
                                 </div>
                              ))}
                           </div>
                        </div>
                     ...
                     {activeTab === 'escalation' && (
                        <div className="space-y-8">
                           <h4 className="text-[10px] font-mono font-black uppercase text-zinc-500 tracking-widest">Escalation Nodes</h4>
                           <div className="space-y-4">
                              {[
                                 { label: 'High-Value Lead', status: 'MONITORING', color: 'bg-primary' },
                                 { label: 'Technical Complex', status: 'READY', color: 'bg-amber-500' },
                                 { label: 'Priority Escalation', status: 'ARMED', color: 'bg-red-500' },
                              ].map((e, i) => (
                           ...
                           <div className="p-6 bg-primary/5 border border-primary/20 rounded-2xl flex items-center gap-4 group cursor-pointer hover:bg-primary/10 transition-all">
                              ...
                              <p className="text-[10px] font-mono text-zinc-400 italic">Context transfer quality at <span className="text-white font-bold">100%_SECURE</span> during human handoff testing.</p>
                           </div>
                        </div>
                     )}
                     ...
                     {activeTab === 'optimization' && (
                        <div className="space-y-8">
                           <h4 className="text-[10px] font-mono font-black uppercase text-zinc-500 tracking-widest">AI Engine Performance</h4>
                           <div className="h-56 w-full">
                              <ResponsiveContainer width="100%" height="100%">
                                 <AreaChart data={CONTINUITY_FLOW}>
                                    ...
                                    <YAxis hide />
                                    <Tooltip contentStyle={{ backgroundColor: '#000', border: 'none', borderRadius: '12px' }} />
                                    <Area type="monotone" dataKey="continuity" stroke="#22d3ee" fill="#22d3ee" fillOpacity={0.1} />
                                 </AreaChart>
                              </ResponsiveContainer>
                           </div>
                           <div className="grid grid-cols-2 gap-4">
                              <div className="p-6 bg-white/[0.02] border border-white/5 rounded-3xl">
                                 <p className="text-[9px] font-font text-zinc-500 uppercase mb-1">Consistency</p>
                                 <p className="text-2xl font-display text-white">99.8%</p>
                              </div>
                              <div className="p-6 bg-white/[0.02] border border-white/5 rounded-3xl">
                                 <p className="text-[9px] font-font text-zinc-500 uppercase mb-1">Grounding</p>
